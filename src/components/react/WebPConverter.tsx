...
function generateId(): string {
  return `webp-${++idCounter}-${Date.now()}`;
}
        ...
        canvas.toBlob(
          (blob) => {
            ...
            if (blob) {
              ...
              reject(new Error('Conversion produced empty result'));
            }
          },
          mimeType,
          qualityValue,
...
export default function WebPConverter({ labels }: WebPConverterProps) {
    ...
    (files: File[]) => {
      const newItems: (ConversionResultItem & { file: File })[] = files.map((file) => {
        ...
        return {
          ...
          status: isWebP ? ('pending' as const) : ('error' as const),
          error: isWebP ? undefined : labels.errorNotWebp,
          file,
        };
      });
  ...
  const convertFiles = async (
    ...
    for (const item of filesToConvert) {
      ...
      try {
        const { blob, previewUrl } = await convertWebP(item.file, fmt, qual);
        const ext = fmt === 'jpg' ? '.jpg' : '.png';
        const baseName = item.fileName.replace(/\.webp$/i, '');
